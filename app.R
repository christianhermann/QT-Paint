...
ui <- function(request) {
  fluidPage(titlePanel("QT-Paint"),
                ...
                sidebarLayout(
                  ...
                  ,
                  mainPanel(
                    actionButton("Import_ASC", "Import .ASC"),
                    actionButton("Import_CSV", "Import .CSV"),
                    actionButton("Import_R", "Import .rData"),
                    actionButton("Save_CSV", "Save .CSV"),
                    actionButton("Save_R", "Save .rData"),
                    # save_ggplot_ui("save"),
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (length(CTcurve) > 0) {
      ...
      active_plot <-
        ggplot() + geom_line(data = plot_data, aes(
          x = time,
          y = meanValue,
          colour = meanType
        ),
        size = input$PlotLineSize) +
        theme_CTcurve(input$FontSize_Axis, input$FontSize_Axis - 1) + geom_hline(yintercept = 0) + ylab(TeX("Curr. dens. (pA pF$^{-1})$")) +
  ...
  observeEvent(input$Import_ASC, {
    dir_asc <-
#dlg_open(title = "Select asc", filters = matrix(c("ASC files", "*.asc"), 1, 2))$res
      dlg_open(title = "Select asc",  1, 2)$res
  ...
  observeEvent(input$Save_CSV, {
    write.csv(CTcurve, paste0(dlgSave(title = "Save Plot")$res, ".csv"), row.names = F)
  })

  
  observeEvent(input$Save_R, {
    save(CTcurve, file = paste0(dlgSave(title = "Save Plot")$res, ".rData"))
  })

  observeEvent(input$Import_CSV, {
    CTcurve <- read_csv(dlg_open(title = "Select CSV", filters = matrix(c(
      "CSV files", "*.csv"
    ), 1, 2))$res)
  ...
  observeEvent(input$Import_R, {
    load(dlg_open(title = "Select .rData", filters = matrix(c(
      "rdata files", "*.rData"
    ), 1, 2))$res)
  ...
  onRestore(function(state) {
    ...
    #     ),
    #     size = input$PlotLineSize) +
    #     theme_CTcurve() + geom_hline(yintercept = 0) + ylab(TeX("Curr$.$ $$dens$.$ $$(pA pF^-^1)")) +
    #     xlab("Time (s)") + scale_color_manual(values = c("black","black"))
    #   
    #   active_plot <-
    #     active_plot + geom_segment(
    ...
    #   }
    #   active_plot <-
    #     active_plot + scale_x_continuous(limits = c(xmin,
    #                                                 xmax),
    #                                      expand = expansion(mult = c(0,
    #                                                                  0))) +  coord_cartesian(ylim = c(ymin, ymax), expand = F)
    #   # active_plot <- active_plot + geom_segment(aes(
    #   #   x = xmin,
    #   #   y = ymin_orig,
    #   #   xend = xmin,
    ...
    #   
    #   active_plot <<- active_plot
    #   output$CTcurvePlot <- renderImage({
